// TtsNetworkServer.swift — updated to use Rokid AI glasses SDK
...
final class TtsNetworkServer: ObservableObject {
    ...
    func broadcastTtsStart(uuid: String) {
        // no text param — nothing to speak
    }
    ...
    func broadcastServiceInfo(bound: Bool, descriptor: String) {
        guard let dev = activeDevice else { return }
        RokidMobileSDK.vui.sendMessage(topic: "serviceinfo", text: String(describing: bound), to: dev)
    }
}

// ── Receive voice commands FROM the glasses ───────────────────────────────────
extension TtsNetworkServer: SDKBinderObserver {
